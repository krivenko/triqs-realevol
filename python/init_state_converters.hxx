...
// --- C++ Python converter for eq_solver_parameters
...
using realevol::eq_solver_parameters;

template <> struct py_converter<eq_solver_parameters> {
 static PyObject *c2py(eq_solver_parameters const & x) {
 ...
 static eq_solver_parameters py2c(PyObject *dic) {
  eq_solver_parameters res;
 ...
 _error:
   fs2 << "\n---- There " << (err > 1 ? "are " : "is ") << err<< " error"<<(err >1 ?"s" : "")<< " in Python -> C++ transcription for the class eq_solver_parameters\n" <<fs.str();
