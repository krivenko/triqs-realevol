...
template<typename F>
auto init_array(segment_mesh const& m, F && f) -> triqs::arrays::array<decltype(f(.0)), 1> {
  triqs::arrays::array<decltype(f(.0)), 1> res(m.size());
  for(int i : range(m.size())) res(i) = f(m[i]);
  return res;
}
...
time_interp ti7(m, 1.9+2.8_j);
time_interp ti8(m, init_array(m, [](double t){ return t*t + 1_j*(t + 1); }));
...
TEST(time_interp, RealImag) {
  ...
  auto ti5_conj_ref = time_interp(m, init_array(m, [](double t){ return t + 1 - 1_j*t*t; }));
...
TEST(time_interp, Addition) {
  ...
  time_interp ti1pihalf = ti1 + 0.5_j;
  time_interp ihalfpti2 = 0.5_j + ti2;
  ...
  for(int i = 0; i < N_t; ++i) {
    ...
    EXPECT_CLOSE(TI1_res[i]+0.5_j, ti1pihalf(t));
    EXPECT_CLOSE(0.5_j+TI2_res[i], ihalfpti2(t));
  }
}
...
TEST(time_interp, Subtraction) {
  ...
  time_interp ti1mihalf = ti1 - 0.5_j;
  time_interp ihalfmti2 = 0.5_j - ti2;
  ...
  for(int i = 0; i < N_t; ++i) {
    ...
    EXPECT_CLOSE(TI1_res[i]-0.5_j, ti1mihalf(t));
    EXPECT_CLOSE(0.5_j-TI2_res[i], ihalfmti2(t));
  }
}
...
TEST(time_interp, Multiplication) {
  ...
  time_interp ti1ppihalf = ti1 * 0.5_j;
  time_interp ihalfppti2 = 0.5_j * ti2;
  ...
  for(long i = 0; i < N_t; ++i) {
    ...
    EXPECT_CLOSE(TI1_res[i]*0.5_j, ti1ppihalf(t));
    EXPECT_CLOSE(0.5_j*TI2_res[i], ihalfppti2(t));
  }
}
...
TEST(time_interp, Division) {
  ...
  time_interp ti1dihalf = ti1 / 0.5_j;
  time_interp ihalfdti2 = 0.5_j / ti2;
  ...
  for(int i = 0; i < N_t; ++i){
    ...
    EXPECT_CLOSE(TI1_res[i] / 0.5_j, ti1dihalf(t));
    EXPECT_CLOSE(1_j*halfd_TI2_res[i], ihalfdti2(t));
  }
}
...
TEST(time_interp, try_reduce_to_constant) {
  ...
  time_interp ti(m, init_array(m, [](double t){ return double(t > 50); }));
