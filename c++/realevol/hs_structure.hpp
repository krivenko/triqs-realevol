...
struct hilbert_space_structure {
 ...
 std::vector<bool> classify_subspaces(HamiltonianType const& op, ClassifyPredicate classify_pred) const {
  ...
  // Make a vector of subspace-to-subspace connections generated by a given monomial
  std::vector<int> make_monomial_connections(monomial_t const& m) const {
    std::vector<int> res(sub_hilbert_spaces.size());
    std::iota(res.begin(), res.end(), 0);
    for(auto it = m.rbegin(); it != m.rend(); ++it) {
      for(int n = 0; n < sub_hilbert_spaces.size(); ++n) {
        if(res[n] == -1) continue;

        int linear_index = fops.pos(it->indices, it->stat);
        auto const& conn = (it->dagger ? creation_connection : annihilation_connection)[it->stat];
        res[n] = conn(linear_index, res[n]);
      }
    }
    return res;
  }
