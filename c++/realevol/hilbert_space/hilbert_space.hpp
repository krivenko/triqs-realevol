...
class hilbert_space {
 ...
 std::vector<int> boson_offsets_;  // For each boson, offset in bits
 std::vector<int> bits_per_boson_; // For each boson, length in bits
 ...
 /**
   ...
   @param bits_per_boson Number of binary digits per bosonic degree of freedom
 */
 hilbert_space(fundamental_operator_set const &fops, std::vector<int> const& bits_per_boson = {}) :
  ...
  for(int l : bits_per_boson_) {
   boson_offsets_.emplace_back(bit);
   bit += l;
  }
  dim = 1ull << bit;
 }

 /// Return the total number of the Fock states in this space
 /**
 ...
 */
 ...
 /**
   @return For each boson, offset in bits
  */
 std::vector<int> const& boson_offsets() const { return boson_offsets_; }

 /// Access bit length of the bosonic states
 /**
   @return For each boson, length in bits
  */
 std::vector<int> const& bits_per_boson() const { return bits_per_boson_; }
 ...
 */
 bool operator==(hilbert_space const& hs) const {
  return dim == hs.dim &&
         boson_offsets_ == hs.boson_offsets_ &&
         bits_per_boson_ == hs.bits_per_boson_;
 }
 ...
 */
 ...
 /**
   ...
   @param fermion_indices A set of index sequences (see `indices_t` in [[fundamental_operator_set]]) for the fermionic operators
   @param boson_indices A set of index sequences (see `indices_t` in [[fundamental_operator_set]]) for the bosonic operators
   @return Fock state
 */
 ...
 /// Return name of the HDF5 format
 /**
 ...
 */
 friend void h5_write(h5::group fg, std::string const& name, hilbert_space const& hs) {
  ...
  h5_write(gr, "boson_offsets", hs.boson_offsets_);
  h5_write(gr, "bits_per_boson", hs.bits_per_boson_);
 }
 ...
 */
 friend void h5_read(h5::group fg, std::string const& name, hilbert_space & hs) {
  ...
  h5_read(gr, "boson_offsets", hs.boson_offsets_);
  h5_read(gr, "bits_per_boson", hs.bits_per_boson_);
 }
