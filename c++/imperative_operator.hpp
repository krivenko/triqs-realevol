...
template <typename ScalarType, typename HilbertType, bool UseMap = false> class imperative_operator {
 ...
 imperative_operator(many_body_operator<scalar_t> const &op, fundamental_operator_set const &fops,
 ...
 static auto apply_if_needed(scalar_t const& x, Args&&... args) -> typename std::result_of<scalar_t(Args...)>::type
 {
    return x(std::forward<Args>(args)...);
 }
 static auto apply_if_needed(scalar_t const& x) -> scalar_t
 {
     return x;
 }
 ...
 StateType operator()(StateType const &st, Args... args) const {
  ...
  for (int i = 0; i < all_terms.size(); ++i) { // loop over monomials
   ...
   foreach(st, [M, &target_st,hs,args...](int i, typename StateType::value_type amplitude) {
    ...
    target_st(ind) += amplitude * apply_if_needed(M.coeff,args...) * (sign_is_minus ? -1.0 : 1.0);
   }); // foreach
  }
  return target_st;
 }
