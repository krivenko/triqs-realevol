...
class simulation : public boost::static_visitor<results_t> {
    ...
    block_mapping_t obs_connections; // Connections generated by the observables
...
private:
    ...
    void partition(fundamental_operator_set const& fops, hilbert_space const& hs, params_t const& p) {
        ...
        for(auto const& obs : p.observables) {
            op_on_space_t op(obs.second,fops);
    ...
    void run_my_jobs(Mesh const& mesh) {
        if(schedules[comm.rank()].empty()) return; // Nothing to do on this MPI node
