...
class simulation : public boost::static_visitor<results_t> {
    ...
    using space_partition_t = space_partition<state<hilbert_space,operator_coeff_t<ComplexOperators>,true>,
                                              imperative_operator<hilbert_space,operator_coeff_t<ComplexOperators>,false>>;

    fundamental_operator_set fops;
    std::vector<sub_hilbert_space> subspaces;
    typename space_partition_t::block_mapping_t obs_connections; // Connections generated by the observables
...
private:
    ...
    void partition(fundamental_operator_set const& fops, hilbert_space const& hs, params_t const& p) {
        ...
        for(auto const& obs : p.observables) {
            typename space_partition_t::operator_t op(obs.second,fops);
            auto conn = SP.find_mappings(op);
    ...
    void distribute_jobs(state<hilbert_space,std::complex<double>,false> const& init_state, params_t const& p) {
        ...
        for(auto it = std::begin(obs_connections); it != std::end(obs_connections); ++it) {
            if(subspace_disp_table[it->first].first == missing_rank ||
               subspace_disp_table[it->second].first == missing_rank) {
                obs_connections.erase(it);
            }
        }
    }
