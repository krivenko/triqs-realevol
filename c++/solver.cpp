...
solver::solver(gf_struct_t const& gf_struct, gf_struct_t const& chi_struct,
 ...
 chi_l = init_block_gf(chi_struct, t_mesh);
 chi_g = init_block_gf(chi_struct, t_mesh);
}
...
void solver::compute_2t_obs(compute_2t_obs_parameters_t const& params) {
 ...
 auto choose_obs =
  [this](worldline_desc_t const& wl, bool verbose, long nwl) -> block_gf_2t_t& {
  if(verbose)
   std::cout << "[Node " << comm.rank() << "] Evaluating world line "
             << nwl;
  if(wl.observable == worldline_desc_t::GreensFunction) {
   if(wl.is_greater) {
   ...
   if(wl.is_greater) {
    if(verbose) std::cout << " (greater susceptibility component)" << std::endl;
    return chi_g;
   } else {
    if(verbose) std::cout << " (lesser susceptibility component)" << std::endl;
    return chi_l;
   }
  }
 };
 ...
 try {
  while(true) {
   ...
   // Interruption?
   if(triqs::signal_handler::received()) {
 ...
 chi_g = mpi_reduce(chi_g, comm, 0, true);
 chi_l = mpi_reduce(chi_l, comm, 0, true);
